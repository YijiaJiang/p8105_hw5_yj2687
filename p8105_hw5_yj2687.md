...
<img src="p8105_hw5_yj2687_files/figure-gfm/unnamed-chunk-6-1.png" width="90%" />
...
sim_result = tibble(mu = c(0:6)) %>%
  mutate(output = map(.x = mu, ~sim_result(.x))) %>%
  unnest(output)
```
...
<img src="p8105_hw5_yj2687_files/figure-gfm/unnamed-chunk-8-1.png" width="90%" />
...
sim_result %>% 
  ...
  scale_x_continuous(limits = c(0,6), breaks = seq(0,6,1)) +
  scale_y_continuous(limits = c(-0.1,6), breaks = seq(0,6,1)) +
  labs(
...
<img src="p8105_hw5_yj2687_files/figure-gfm/unnamed-chunk-9-2.png" width="90%" />
